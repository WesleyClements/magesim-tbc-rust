...
export default {
  ...
  methods: {
    ...
    draw() {
      ...
      // Mana
      let d = [];
      const manaSmooth = true;
      if (manaSmooth) {
        for (let i = 0; i < this.result.log.length; i += 1) {
      ...
      // CDs
      for (let i = 0; i < this.cds.length; i += 1) {
      ...
      // Trinkets
      const trinketColors = ['rgba(255,255,255,0.6)', 'rgba(160,60,160,0.6)'];

      let t = 0;
      for (let i = 0; i < this.trinkets.length; i += 1) {
      ...
      // Mana gained
      // const self = this;
      // for (let i = 0; i < this.mana_gains.length; i += 1) {
      //   start = _.filter(
      //     this.result.log,
      //     (a) => a.text.indexOf(` mana from ${self.mana_gains[i].title}`) > 0,
      //   );
      //   for (let j = 0; j < start.length; j += 1) {
      //     data.datasets.push({
      //       data: [
    ...
    drawImages() {
      ...
      const buffs = _.concat(this.cds, this.trinkets);
      for (let i = 0; i < buffs.length; i += 1) {
        ...
        if (buffs[i].img) {
          for (let j = 0; j < start.length; j += 1) {
            ...
            const im = new Image();
            im.onload = ((xx, yy, img) => () => setTimeout(
              () => {
      ...
      for (let i = 0; i < this.mana_gains.length; i += 1) {
        start = _.filter(this.result.log, (a) => a.text.indexOf(` mana from ${self.mana_gains[i].title}`) > 0);
